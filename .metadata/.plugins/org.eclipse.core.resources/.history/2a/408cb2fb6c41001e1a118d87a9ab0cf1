...
public class subOfferServiceImpl implements ISubOfferService {
	...
	//Get SubOffers who have Parent Relation Optional using OfferID.
	@Override
	...
	//Get Amount to be Paid using offerId and String of suboffer IDs
	@Override
	public ResponseEntity<?> getAmountToBePaid(Long offer_id, String suboffers) {
		
		//Get all the suboffer Ids in array.
		String[] variableArray = suboffers.split(",");
        double amount = 0;
        
        for (String variable : variableArray) {
        	
        	//If No suboffer is present.
        	if(variable == "0") {
        		break;
        	}
        	SubOffer suboffer = this.getSubOfferById(Long.parseLong(variable));
        	//Check subOffer Id is present under Given OfferID.
        	if(offer_id ==suboffer.getOffer().getId()) {
        		
        		//User can send sub offer Id with Parent Relation D aor M
        		//So just to filter out.
        ...
        //Now Get the sob offers who have parent relation D and M
        //becase this are the default and mandatory sub offers which comes with offer.
        List<SubOffer> offers = this.getSubOffersWithParentRelation(offer_id);
        
        //Now add the price of D and M
	...
	//Method to get sub offers with D and M relation.
	@Override
